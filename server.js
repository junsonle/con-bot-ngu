                            ...
                            if (orderLongId !== -1) {
                                ...
                                binance.futuresOrderStatus(configs.symbol, {orderId: `${orderLongId}`}).then(order => {
                                    if (order.status === 'NEW') {
                                        ...
                                        openLong(Math.round(Math.min(botLong, price)) - configs.range, configs.amount);
                                        openShortM(Math.round(Math.min(botLong, price)) - configs.range, configs.amount);
                                    }
                                });
                            }
                            // open long market
                            // if (orderLongMId === -1 && Math.abs(position[1].positionAmt) >= position[0].positionAmt) {
                            ...
                            if (orderShortId !== -1) {
                                ...
                                binance.futuresOrderStatus(configs.symbol, {orderId: `${orderShortId}`}).then(order => {
                                    if (order.status === 'NEW') {
                                        ...
                                        openShort(Math.round(Math.max(topShort, price)) + configs.range, configs.amount);
                                        openLongM(Math.round(Math.max(topShort, price)) + configs.range, configs.amount);
                                    }
                                });
                            }
                            // open short market
                            // if (orderShortMId === -1 && Math.abs(position[1].positionAmt) <= position[0].positionAmt) {
